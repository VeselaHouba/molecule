def main(ctx):
  ...
  oses = docker_images
  for server_os in oses:
    for client_os in oses:
      pipelines.append(step_docker(server_os,client_os))
...
def step_docker(server_os,client_os):
  return {
    "kind": "pipeline",
    "depends_on": [
        "linter",
    ],
    "name": "molecule-%s-%s" % (server_os,client_os),
    "services": [
      {
        "name": "docker",
        "image": "docker:dind",
    ...
    "steps": [
      {
        ...
        "commands": [
          ...
          "ansible --version",
          "molecule --version",
          "MOLECULE_IMAGE_SERVER=geerlingguy/docker-%s-ansible" % server_os,
          "MOLECULE_IMAGE_CLIENT=geerlingguy/docker-%s-ansible" % client_os,
...
def step_publish(oses):
  ...
  for server_os in oses:
    for client_os in oses:
      deps.append("molecule-%s-%s" % (server_os,client_os))
  return {
    "kind": "pipeline",
    "depends_on": deps,
    "name": "publish",
